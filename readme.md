...
这个功能在某些场景很有用。比如我的电脑和手机的obsidian使用Remotely Save同步，且仅同步文件而不同步插件配置，但是数据表格的配置信息是存储在插件的data.json中，也就是说它不会被同步。所有通过导出、导入功能实现2个库之间的插件配置信息同步。

假设现在我想把 A 库中的数据表格配置信息同步到 B 库中，那么需要这样做：

1. 在 A 库中导出（插件会导出配置信息到根目录下的`YAML Database Config.json`）
2. 将 A 库中的`YAML Database Config.json`文件同步到 B 库中
3. 在 B 库中导入（插件会从根目录的`YAML Database Config.json`导入表格配置信息）
