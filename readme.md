...
* [安装](#安装)
* [使用方法](#使用方法)
  * [代码块：表格编辑YAML](#代码块：表格编辑YAML)
  * [命令：还原上一步操作](#命令：还原上一步操作)
* [版本更新日志](#版本更新日志)
...
> 之前的插件中的表格是纯手写的，在显示多条数据时有很大的性能问题，而且难以实现很多想要实现的复杂功能，于是开始重构代码，参考qq群里峰华老哥开发的[windily-cloud/obsidian-AGtable](https://github.com/windily-cloud/obsidian-AGtable)插件使用的[AG Grid](https://www.ag-grid.com/)，目前的表格使用体验比原本要好很多。
>
...
- 上方的操作区域：`新建`、`搜索`、`编辑列`、`表格设置`、`刷新`（后面3个按钮尽在鼠标移动到操作区域才会显示）
...
3. `编辑列`：点击该按钮后，将鼠标移至表头的某一列并点击右键弹出菜单，操作完成后您可能需要点击`应用变更`来保存更改，现支持以下操作：
...
4. `表格设置`：点击之后会打开面板，操作即可，记得操作完后点击`应用变更`
   1. `属性显隐控制`：控制属性的显示和隐藏
...
- 代码块中的数据是JSON字符串，它包含以下几个参数，在此说明以方便使用：
  - `id`：输入一个在当前文档中所有的 yamledit 代码块中独一无二的 id
...
- 注意：表格每次渲染前都会进行初始化，会进行数据更新：
  - 库中的文档YAML属性数据 →单向→ 表格内插件生成的JSON数据
  - 这就会出现一些问题，例如若当前表格指定的文件夹路径失效，那么表格会渲染失败，这时候您可以手动更改代码块中的JSON数据中的`folder`参数以使表格成功渲染

### 过滤和排序

在`编辑列`未被触发的情况下，点击表头进行排序和过滤操作。

**多条件排序**：按住`Ctrl`点击表头即可

> 为了存储当前过滤和排序参数，表格会重新渲染，这会导致表格闪烁。
