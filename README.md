...
1. 创建代码块`yamledit`并输入参数（示例见[代码示例](#代码示例)）
   1. **（可选）输入要显示的属性**
      1. 语法格式：`porp:属性名1,属性名2……`
         1. 以`prop:`开头；
         2. 后面每个属性之间以英文半角逗号`,`隔开；
         3. 表格显示的属性顺序和您输入的属性顺序是一致的。
   2. **（可选）输入排序条件**，参数与参数之间使用英文半角冒号`:`隔开
      1. 语法格式：`sort:属性名1,asc/desc:属性名2,asc/desc……`
         1. 以`sort:`开头
         2. 后面的每个排序参数的组成为`属性名,asc`或`属性名,desc`，
         3. 如果您相对文件名排序，那么您可以使用排序参数`文件,asc`或`文件:desc`
         4. 排序参数的优先级和您输入排序参数的顺序一致，最前面的排序参数优先级最高。
      2. 示例：`sort:文件,asc:ctime:desc`
         1. 对文件名升序排序
         2. 文件名同名的话对ctime执行降序排序
   3. **输入筛选条件**，参数与参数之间使用英文半角冒号`:`间隔开来。
      1. 语法格式：`条件参数1:条件参数2:条件参数3`（这个只是一个条件，新条件的话换行即可）
         1. `条件参数`详见：[筛选条件说明](#筛选条件说明)
         2. 不输入筛选条件的话就是默认筛选全部文档
         3. 多个筛选条件换行即可（多个条件之间均为与关系）
      2. 示例：`yaml:包含:ctime`
         1. 筛选yaml中包含ctime属性的文档
2. 操作表格编辑属性
...
在示例4的基础上增加排序`sort:国家,desc:演员,asc`，排序条件的顺序就是它的优先级，这里`国家,desc`的优先级就要比`演员,asc`的优先级高。
