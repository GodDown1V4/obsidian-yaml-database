...
- 批量编辑YAML：![image](https://user-images.githubusercontent.com/39726621/160730190-85380500-7e90-4f3c-ac01-7061f9a04466.png)
...
5. 到这里就完成批量修改操作了。
6. 批量编辑设置项
   1. `禁止删除和修改的名称属性`：对于十分重要的yaml属性值，为了避免对这些重要的属性值造成误删、误改，可以将他们添加到此。（一行一个属性，多个属性请换行）![image](https://user-images.githubusercontent.com/39726621/160730853-8372818b-8bcc-4f66-afa7-af71987895fb.png)
   2. `忽略的文件或文件夹路径`：输入基于库的路径，路径开头不要加`/`，（一行一个路径，多个路径请换行）![image](https://user-images.githubusercontent.com/39726621/160730894-f701ee0a-775a-47ad-8ac9-292885251f32.png)
...
5. 表格编辑设置项：
   1. `禁止修改的属性名称`：在这里输入您想要禁止在表格中编辑的YAML属性。（一行一个属性，多个属性请换行）![image](https://user-images.githubusercontent.com/39726621/160731083-e1e18e2d-3757-4b91-bd69-22bd19eec50c.png)
   2. `隐藏的属性名称`，在这里输入您想要默认隐藏的属性。（一行一个属性，多个属性请换行）（如果您在代码块中`prop:`输入的要显示的属性中包含表格中隐藏的属性，那么该属性仍会被显示。换句话说，这个选项只在你的代码块中没有`prop:`参数时起作用）![image](https://user-images.githubusercontent.com/39726621/160731246-a43163f1-a0fe-4d6c-aa13-f48dc2295168.png)
