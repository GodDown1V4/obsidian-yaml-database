...
* [使用方法](#使用方法)
  ...
  * [yaml 表格操作](#yaml-表格操作)
    ...
    * [操作区域](#操作区域)
    * [表格区域](#表格区域)
  * [(高级操作) yamledit 代码块](#高级操作-yamledit-代码块)
    * [语法](#语法)
      * [参数说明](#参数说明)
      * [注意事项](#注意事项)
    * [相关设置项](#相关设置项)
...
2. 表格编辑YAML
   （编辑**某个文档的某个属性值**）
   1. 点击`筛选`筛选文档
   1. 点击`属性`指定显示的属性及顺序，还可以指定属性的名称和数据类型
   1. 点击`↑↓`设置排序条件，支持多条件排序，排序优先级即为条件的顺序
   1. 点击`搜索`对当前表格内容进行搜索
   1. 点击`刷新`重新读取缓存刷新表格
   2. 点击表格单元格来改变某个文档的某个属性值
...
可以看到`yamledit`代码块种一共是有3种数据：

|                    | 基础操作                                                     | 高级操作                                                     |
| ------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `id`<br />（必选） | 您需要为当前yaml表格输入一个id以使表格编辑正常运行。<br />`id:xxxxxxxxxx`（**只要这个id不和当前文档内的其它yamledit的id相同即可**） |                                                              |
| `prop`（可选）     | 指定要显示的属性，属性与属性之间使用英文半角逗号`,`隔开。例如显示属性`A`,`B`：<br />`prop:A,B`<br />一个代码块种只允许存在一个`prop`行<br />（指定`type.`类型时请谨慎操作！详见注意事项） | ①指定属性`A`显示为名称`b`<br />`prop:A:name.b`<br />②指定属性`A`的数据类型为`c`<br />`prop:A:type.c`<br />③指定属性`A`显示为名称`a`，且数据类型为`c`<br />`prop:A:name.b:type.c` |
| `sort`（可选）     | 指定排序方式，多个排序之间使用英文半角逗号`,`隔开。例如按属性`A`升序排序：<br />`sort:A`<br />一个代码块种只允许存在一个`sort`行 | ①按属性`A`降序、`B`升序排序<br />`sort:A:desc,B:asc`<br />②按`文件名称`降序排序<br />`sort:文件:desc`<br /> |
| `条件`（可选）     | 筛选文档以显示（若不输入该参数则默认显示全部文档）。筛选条件详见[筛选条件说明](#筛选条件说明)。<br />一个筛选条件占一行，想添加新的筛选条件换行即可 |                                                              |

目前`prop`中的`type.`支持以下几种属性类型：

- [x] text（默认类型，可显示所有类型的值）
- [x] checkbox（选框）
- [x] date（日期）
- [x] time（时间）
- [x] number（数字）
...
在示例4的基础上增加排序`sort:国家:esc,演员:asc`，排序条件的顺序就是它的优先级，这里`国家:desc`的优先级就要比`演员:asc`的优先级高。
...
在示例6的基础上更改，想在指定`演员`的数据类型为`text`（实际上默认就是text类型）
